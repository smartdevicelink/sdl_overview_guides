...
Heartbeat timeout for this application, deprecated in [Protocol Version 4](https://github.com/smartdevicelink/protocol_spec#45-heartbeat).
...
Defines a list of potential serviceNames that an app can publish with this service type. If the serviceName field in the app's `PublishAppService.appServiceManifest` does not match one of the values in this list, the message will be rejected. If omitted, no restrictions will be applied to serviceName.
...
The [function ID](https://github.com/smartdevicelink/sdl_core/blob/master/src/components/interfaces/MOBILE_API.xml) of the potential handled RPC. Can be an function ID which was introduced after Core's local RPC spec version.
